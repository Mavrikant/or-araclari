...
## DÖNGÜ #37 — 2026-07-10 / 07-11

**Yapılan:** F-CDS-NEH — Johnson akış-tipi çizelgeleme aracına Campbell-Dudek-Smith (1970) ve Nawaz-Enscore-Ham (1983) sezgiselleri ikinci ve üçüncü algoritma olarak eklendi. JPS'i A*'a eklediğim desenin birebir tekrarı: yeni saf kütüphane + aynı UI'da algoritma radyosu + makine sayısı seçici (2..8) + algoritma-özel sonuç paneli. Johnson m ∈ {2, 3} kısıtından m ≥ 4 desteğine geçildi.

**Detay:**
- `src/lib/flow-shop-m.ts` (+310) — genelleştirilmiş m-makina permütasyon flow-shop simülatörü (`simulateFlowShopM`), `solveCDS()` (m-1 alt problem, her biri Johnson kuralıyla, orijinal m makinada Cmax'ı en küçüğü seçilir), `solveNEH()` (Σp azalan sırada ekleme + tüm pozisyon deneme + ilk-pozisyon-kazanır tie-break + ekleme adım izi), `bruteForceOptimumM()` (n ≤ 8). `johnsonPermutation()` iç yardımcı yeniden implementasyon (johnson.ts ile döngüsel bağlantı yok).
- `src/lib/flow-shop-m.test.ts` (+307, **29 vitest**) — hand-verified 4×3 örneği (sıra [J2, J3, J1, J4] → Cmax 26, [J3, J2, J1, J4] → Cmax 25), CDS m=2 → Johnson optimum eşitliği (Taha 5-iş = 28), CDS k=1/k=2 alt problemlerinin α/β/grup birebir doğrulaması, NEH tam adım-adım ekleme izi (aday Cmax listesi bit-bit hand-computed), brute-force karşılaştırması (CDS ≤ 1.5·OPT, NEH ≤ 1.2·OPT), tie-break invariantı, kısmi sıra kabulü (NEH'in iç kullanımı için), validation (empty/negative/dim mismatch/out-of-range/duplicate).
- `src/pages/araclar/johnson-akis-tipi-cizelgeleme.astro` (~697 satır, +495) — tam yeniden yazım: 7 tuşlu makine sayısı radio grubu (2..8, has-checked stil), algoritma radio grubu (Johnson/CDS/NEH), m ≥ 4'te Johnson radyosu `disabled` + gri arka plan + info-note, hazır örnek 3'e çıkarıldı (Taha 2M / 3M cond-sat / **NEH 5-iş 4-makina**), dinamik iş tablosu sütunları (label + m adet süre + delete), genelleştirilmiş m-makine Gantt SVG (M₁..Mₘ satırları), sıra tablosu tüm makina aralıkları, metrikler (n, m, toplam boş, doluluk %), algoritma-özel detay panel: Johnson U/V (korundu), **CDS alt problem tablosu** (her k için Johnson sırası + Cmax, seçilen k emerald vurgulu), **NEH ekleme adım izi** (Σp sıralaması + adım-adım aday Cmax badge'leri, seçilen pozisyon emerald). Hidrasyon inline `<script>`, plotly yok. localStorage key `johnson-state-v1` → `johnson-state-v2` (v2 şema: `{machines: 2..8, algo: 'johnson'|'cds'|'neh', jobs: [{label, times: string[]}]}`).
- `src/data/tools.ts` — başlık `"Akış-Tipi Çizelgeleme — Johnson · CDS · NEH"`, shortTitle `"Flow-Shop (Johnson · CDS · NEH)"`, description üç algoritmayı ve algoritma-özel panelleri içerir.
- `src/content/rehberler/johnson-akis-tipi-cizelgeleme.mdx` — başlık `"Akış-Tipi Çizelgeleme: Johnson, CDS ve NEH"`, description CDS/NEH ekli (198 char ≤ 200), "Aracımızda ne var?" bölümü üç algoritmayı ve panelleri detaylı listeler.
- `public/og/johnson-akis-tipi-cizelgeleme.png` — yeni başlıkla yeniden üretildi (82.7 KB). Diğer 21 OG kartı byte-identical (deterministik render).
- `.claude/launch.json` — yeni: Astro dev-server port 4321 config (Browser MCP smoke testi için).

**Tasarım notu:** Mobil viewport (375×812) doğrulamasında ilk seferde horizontal overflow tespit ettim (`document.scrollWidth = 545 > clientWidth = 375`). Kök neden HTML `<fieldset>` elementinin varsayılan `min-width: min-content` kuralıydı — 4+ makina için jobs table'daki `w-20` input'lar fieldset'i geniş tutuyor, `overflow-x-auto` sarmalayıcısı olmadan taşıyordu. Fix: `min-w-0` sınıfını (a) form, (b) tüm 3 fieldset, (c) output section'a ekledim. Bin Packing sayfası bu problemi yaşamıyor çünkü textarea kullanıyor. `<fieldset>` gotcha'sı yaygın; MST/Steiner gibi başka geniş-tablo araçlarda da olabilir — bir sonraki cycle'da hızlı bir mobil audit yapılabilir. NEH ekleme izinde her adımın aday Cmax badge'lerinin uzunluğu m×n = 40+ blok'a kadar çıkabildiği için `flex flex-wrap` yerine inline `mr-1 inline-block` chip yapısı kullandım — mobilde chip'ler doğal olarak sarılıyor.

**Kalite kapıları:** check ✓ (0/0/0, **128 dosya**) · test ✓ (**741/741**, +29, **35 dosya**) · build ✓ (**72 sayfa**, 6.04s) · Mobil viewport 375×812 doğrulaması: `bw = vw = 375`, taşma yok · Browser MCP UI smoke test: 5×4 NEH sample → NEH Cmax=33 (sıra E → B → D → C → A) · CDS Cmax=35 (NEH beats CDS bekleneni doğruladı) · 2M Taha → Johnson optimum=28 (sıra J4 → J2 → J5 → J3 → J1) · m=6 seçiminde Johnson radio auto-disabled + algo auto-switch NEH'e.

**Yayın:** PR [#74](https://github.com/Mavrikant/or-araclari/pull/74) squash-merge edildi (114e3ef). **İlk deploy run 29121820616 KIRMIZI** — `[vite] ✗ Build failed: Could not parse expression with oxc: Invalid Unicode escape sequence (mdx-jsx:unexpected-character)`. Kök neden: F-CDS-NEH merge'inden ~45dk önce Dependabot PR [#63](https://github.com/Mavrikant/or-araclari/pull/63) (`chore(deps): bump esbuild, @astrojs/mdx and astro`) auto-merge ile Astro 6 → **Astro 7** major upgrade'ini pushlamış; Astro 7'nin yeni oxc MDX parser'ı 5+ rehberdeki LaTeX escape sequence'lerini (`\sqrt`, `\text`) reddediyor. BACKLOG.md içinde `Q-AUDIT-ESBUILD` bu upgrade'i açıkça **🔴 KIRMIZI (insan onayı ister)** olarak işaretliydi — Dependabot policy'yi göz ardı etti. Ayrıntı → [INCIDENTS](INCIDENTS.md).

**Hotfix:** `hotfix/revert-astro7` dalında `git revert 4335cae` ile Dependabot bump'ı geri aldım; yerelde `npm install` + `npm run build` (5.75s) + `npm run test` (741/741) doğrulaması. PR [#75](https://github.com/Mavrikant/or-araclari/pull/75) squash-merge (0babe64) → deploy run **29142324139 YEŞİL** (8s deploy). `curl https://karaman.dev/or-araclari/araclar/johnson-akis-tipi-cizelgeleme/` → 200; sayfada `CDS`, `NEH`, `Campbell-Dudek`, `Nawaz-Enscore` ibareleri görünür.

**İşaret:** 🟡 SARI — hotfix revert (Dependabot dep bump'ının geri alınması). Sarı çünkü package.json + package-lock.json değişti; ama BACKLOG'un ihlal edilen 🔴 policy'sini restore ediyor. F-CDS-NEH işi kendi başına saf yeşildi (yeni araç + rehber genişletmesi, yeni bağımlılık yok, i18n bozmadı, statik-dışı yok). Yeni backlog kalemleri açıldı: **Q-DEPENDABOT-GUARD** (🔴 `.github/dependabot.yml`), **F-ASTRO7-MIGRATE** (🔴 major sürüm göçü), **F-STRIP-PACKING** (yeni araç fikri), **F-JOHNSON-BRUTE** (küçük n için "optimum sağlama" UI).

**Sıradaki:** Backlog'da 🟢 yeşil açık iş: F-STRIP-PACKING (2D Bin Packing uzantısı, M efor) ya da F-JOHNSON-BRUTE (küçük UI cila, S efor). F-STRIP-PACKING kombinatoryel kategoriyi zenginleştirir ve F-BIN-PACKING tazeliğinden yararlanır (aynı algoritma ailesi FFDH/BFDH/NFDH); bir sonraki döngüde önce onu ele almak makul.

---

## DÖNGÜ #36 — 2026-07-10

**Yapılan:** F-BIN-PACKING — Bin Packing (Kutu Paketleme) Çözücü ve uzun-form Türkçe sezgisel/yaklaşım analizi rehberi yayınlandı. Kombinatoryel kategoride Knapsack yanına ikinci araç. Beş klasik sezgisel (NF, FF, FFD, BF, BFD) + iki alt sınır (L₁ hacim, L₂ büyük öge) + renkli SVG kutu görselleştirmesi + karşılaştırma tablosu — hepsi tarayıcıda saf JS ile.

**Detay:**
- `src/lib/bin-packing.ts` (+270) — saf algoritma: 5 sezgiselin ortak `PackedItem`/`Bin` yapısı üzerinde, floating-point tolerant `fits()` (1e-9), `place()` yardımcısı, stable decreasing sort (ties → `originalIndex` — aynı boyutlu ögelerde orijinal sıra korunur), her algoritmaya özel iç fonksiyon (`nextFit`, `firstFit`, `bestFit`); MAX_ITEMS=500 üst sınır, `size > B` girildiğinde erken hata. Sonuçta `binCount`, `lowerBoundL1 = ⌈Σ/B⌉`, `lowerBoundL2 = |{sᵢ > B/2}|`, `lowerBound = max(L₁, L₂, 1)`, `ratio = binCount/lowerBound`, `averageFill = Σsᵢ/(binCount·B)`. `parseItems()` iki biçim kabul eder (yalın sayı satırı otomatik "Öge k" adlanır; TAB + son alan sayı olmak üzere ad + boyut çifti), Türkçe ondalık virgülü.
- `src/lib/bin-packing.test.ts` (+250, **26 vitest**) — `it.each` ile 5 algoritmada partition invariantı (her öge tam bir kutuda + kutu doluluğu ≤ B + kutuların toplamı toplam boyuta eşit), Next Fit klasik `{6,5,4,6,7}/B=10 → {6},{5,4},{6},{7} = 4 kutu` hand-verify, First Fit "Vazirani örneği" 6×0.6/B=1 → 6 kutu (L₁=4 ama L₂=6 → optimum), FFD Johnson 1974 klasik `{6,5,4,3,2,1}/B=10 → {6,4},{5,3,2},{1}` hand-verify + `binCount = L₁ = OPT`, monotonluk (FFD ≤ FF ≤ NF), stable sort (aynı boyutlu ögelerde original order), Best Fit vs First Fit avantajı `{4,7,3,6}/B=10 → FF 3 kutu, BF 2 kutu` hand-verify, alt sınırlar (L₁ hacim, L₂ büyük öge, `L* = max(L₁, L₂, 1)`), validation (boş liste, kapasite üstü öge, negatif/sıfır boyut), parseItems 3 biçim, Vazirani 1.5·OPT sınır kontrolü (12×0.4/B=1 → 6 kutu, L₁=5, ratio 1.2 ≤ 1.5).
- `src/pages/araclar/bin-packing-cozucu.astro` (+310) — mobil-öncelikli form (textarea + kapasite + 5 algoritma radio + "Johnson klasik" / "Zor instans" örnek butonları), emerald vurgulu kutu-sayısı paneli + oran özeti (`binCount / L*`, optimum ise "optimum ✓" yeşil), 4 metrik kartı (öge sayısı, Σsᵢ, L₁, L₂), **SVG paketleme görseli** (her sütun bir kutu, bloklar orijinal indekse göre 8-renkli palet ile boyanır, tabandan yığılır, amber kesikli çizgi = B, y-eksen tick'leri 0/¼/½/¾/B, her kutu altında `used/B · %fill` etiketi), kutu içeriği tablosu (kutu · içerik listesi · dolu/kapasite · doluluk %), **algoritma karşılaştırma tablosu** (aynı girdide 5 sezgiselin sonucu — aktif satır brand-50 vurgulu, oran 1 ise "optimum ✓" yeşil). Hidrasyon inline `<script>`, plotly yok.
- `src/content/rehberler/bin-packing-sezgiselleri.mdx` (+195, ~11 dk okuma) — problem tanımı + NP-zorluk (PARTITION indirgemesi), 5 sezgisel tek tek (NF 2·OPT ispat sezgisi + ne zaman kullan, FF 17/10·OPT Johnson 1973/Dósa-Sgall 2013, FFD 11/9·OPT + 6/9 Johnson 1974/Dósa 2007, BF 17/10·OPT, BFD 11/9·OPT), alt sınırlar L₁ + L₂ formülü ve "6×0.6" örneği (L₂ bağlayıcı olduğunda L₁'i aşar), Johnson klasik `{6,5,4,3,2,1}/B=10` FFD adım adım tablosu (3 kutu = OPT), **Vazirani 1.5·OPT eğitici argümanı** (büyük vs küçük öge ayrımı + Approximation Algorithms 2001), "ne zaman hangi algoritma?" karar tablosu (n < 20 → MILP, offline endüstriyel → FFD/BFD + local search, akış → NF, hassas garanti → Karmarkar-Karp APTAS), modelin sınırları (VSBP, cutting stock, 2D/3D, generalized, online oranları Balogh-Békési-Dosa-Epstein-Levin 2018 ≈ 1.586), sitedeki 3 akraba araca link (Knapsack, Atama, Wagner-Whitin), 5 madde özet, **9 FAQ JSON-LD** (NP-zorluk, FFD > FF sezgisi, L₁ vs L₂ farkı, NF neden 2·OPT ama pratik, APTAS hassasiyeti, FF/BF neden aynı asimptotik, ek local search önerisi, multi-dimensional ayrımı, gerçek dünya uygulamaları).
- `src/data/tools.ts` (+10) — yeni araç kaydı (`kombinatoryel` kategorisi, `guideSlug=bin-packing-sezgiselleri`).
- `public/og/bin-packing-cozucu.png` (82.8 KB) — `npm run og` ile per-tool OG kartı. Diğer 21 OG kartı byte-identical (deterministik render).

**Tasarım notu:** MDX frontmatter'ında `description` alanı 200-char zod sınırını aştı (247 char); build hatasıyla yakalandı ve 197 char'a kısaltıldı — bu, döngü #18'deki 800-char FAQ tuzağının ikiz kardeşi (uzun betimlemeye eğilimli olduğumu hatırlatan bir doğrulama). Kutu görselleştirmesinde ögeleri **orijinal indekse göre** renklendirdim (sıralanmış sırayla değil) — böylece FFD azalan sıralama uygulasa bile kullanıcı "kırmızı blok girdide 3. sırada, mavi 1. sıradaydı" bağlantısını kurabilir. `text/foreignObject` kullanmadan pure SVG tuttum (Lighthouse skoru için); item adları 8 char'da kısaltıldı (`X…`). Karşılaştırma tablosunda aktif algoritma satırı brand-50 ile vurgulandı; "optimum ✓" yeşil rozet oran = 1 durumunda gösteriliyor.

**Kalite kapıları:** check ✓ (0 hata / 0 warning / 0 hint, **126 dosya**) · test ✓ (**712/712**, +26 yeni bin-packing testi, **34 dosya**) · build ✓ (**72 sayfa**, 6.04s — araç sayfası + rehber dahil) · Lighthouse — yeni sayfa mevcut Knapsack/Johnson desenine bire bir uyumlu, hidrasyon stratejisi (inline `<script>`, plotly yok, SVG/Tailwind sadece) ve mobil-öncelikli yerleşim ile (≥95 beklenir).

**Yayın:** PR [#73](https://github.com/Mavrikant/or-araclari/pull/73) squash-merge edildi (b6ea807). `Deploy to GitHub Pages` workflow başarıyla tamamlandı (run 29118076633, 9s deploy job). Canlı: <https://karaman.dev/or-araclari/araclar/bin-packing-cozucu>.

**İşaret:** yok (sırf 🟢 yeşil — yeni araç + rehber, mevcut deseni birebir takip; yeni bağımlılık yok, i18n mimarisi bozulmadı, statik-dışı hiçbir şey yok, mobil-öncelikli SVG `viewBox` + `overflow-x-auto` sarmalayıcı).

**Sıradaki:** F-CDS-NEH (Johnson aracına m ≥ 4 makina için CDS + NEH sezgiselleri, M efor — JPS'i A*'a eklediğim deseni izler; input model dinamikleşir, machine-count input eklenir) backlog'un tek açık 🟢 yeşil işi. Diğer açık işler blokede: Q-CI-CHECK (🔴 workflow), Q-AUDIT-ESBUILD (🔴 major Astro upgrade). Yeni araç fikri: F-STRIP-PACKING (2D strip packing FFDH/BFDH — Bin Packing'in doğal 2D uzantısı) not olarak. Kombinatoryel kategori artık 2 araçlı (Knapsack + Bin Packing) — envanter/graf/olasılık zenginliğine yakın hâle geliyor.
...
**Detay:**
- `src/lib/rop.ts` (+280) — saf JS. Tipler: `RopInput = { demandMean, demandStdDev, leadTimeMean, leadTimeStdDev?, serviceLevel, serviceMode: 'type1' | 'type2', orderQty?, annualDemand?, orderCost?, holdingCost? }`. Ana `computeRop`: (a) validate + finiteOrThrow + servis modu için Q gerekliliği kontrolü, (b) kombine varyans `varianceDL = L·σ_d² + μ_d²·σ_L²` (toplam varyans yasası), (c) Type-I için `z = Φ⁻¹(α)` (Beasley-Springer-Moro), Type-II için hedef `L(z) = (1−β)·Q/σ_DL` bisection ile [-30, 8] aralığında 100 iterasyon, (d) `SS = z·σ_DL`, `R = μL + SS`, (e) EOQ türetimi `√(2DK/h)` opsiyonel + `TC = (D/Q)·K + (Q/2)·h + SS·h`. Yardımcılar: `normalPdf`, `normalCdf` (Abramowitz-Stegun 7.1.26 erf, |ε| < 1.5e-7), `normalInverse` (Beasley-Springer-Moro üç kollu rasyonel, |ε| < 1e-9), `standardNormalLoss` L(z) = φ(z) − z(1 − Φ(z)), `inverseLoss` bisection L monoton azalan olduğu için — z<0'da unbounded olduğundan lo=-30 seçildi. `serviceLevelSweep(input, alphas)`: aynı σ_DL/Q/h ile α ∈ {0.5, 0.75, 0.9, 0.95, 0.975, 0.99, 0.995, 0.999} için R/SS/L(z) tablosu; UI'da duyarlılık paneli.
- `src/lib/rop.test.ts` (+250, **38 vitest**) — (a) Silver-Peterson stili hand-verified: μ=100, σ=10, L=4, α=0.95 → σ_DL=20, z=1.6449, SS=32.9, R=432.9. (b) Chopra-Meindl rasgele tedarik: μ=2500, σ=500, L=2, σ_L=0.5, α=0.9 → σ_DL=1436.14 (σ_L dominant, μ² katsayısı), SS=1840.5. (c) Type-II Q etkisi: Q büyüdükçe aynı β için z azalır, fill rate hedefe eşitlenir (0.98 ± 0.001). (d) Type-I ≠ Type-II asimetri: aynı %95 hedef Type-II'de daha az SS verir. (e) EOQ türetimi: D=10000, K=100, h=2 → Q=1000, TC = (D/Q)·K + (Q/2)·h + SS·h kimlik doğrulaması. (f) monotoniler: α artınca SS ve fill rate artar; σ_L artınca σ_DL μ² ile büyür. (g) 8 validation kenar: demandMean ≤ 0, negatif σ, L ≤ 0, α ∈ {0, 1} sınırları, NaN, Type-II ama Q yok, holdingCost ≤ 0. z-tablo sağlaması (L(0)=φ(0)≈0.3989, L(1.5)≈0.0293 Nahmias tablosu; Cachon-Terwiesch L(-z)=L(z)+z kimliği).
- `src/pages/araclar/rop-emniyet-stogu.astro` (+446) — mobil-öncelikli iki-kolon. Sol form: 4 fieldset (talep: μ_d + σ_d; tedarik süresi: L + opsiyonel σ_L; servis hedefi: Type-I/Type-II radyo + α/β 0-1 alanı; EOQ opsiyonel: Q veya D+K+h), iki hazır örnek butonu (Silver-Peterson deterministik + rasgele tedarik), sıfırla + confirm dialog. Sağ output: yeşil R kartı + `R = μ·L + z·σ_DL = ...` formül satırı, 11 metrik hücresi (E[DL], σ_DL, z, SS, cycle service, fill rate, E[eksik/çevrim], Q, EOQ, çevrim uzunluğu, sipariş/yıl, toplam maliyet — koşullu görünürlük Q verilmediğinde alt hücreler gizli), testere-dişi stok SVG (Q + SS'den doğrusal düşüş → R eşiği → L kadar tedarik süresi düşüşü → sıçra + Q, iki tam çevrim; R kırmızı dashed çizgi, SS amber koridor, zeroLine, x-tick çevrim/L noktaları, y-tick {0, SS, R, Q+SS}), servis seviyesi duyarlılık tablosu (α × z × SS × R × E[eksik]). `localStorage` `rop-state-v1` mod + tüm giriş alanları persist.
- `src/content/rehberler/yeniden-siparis-noktasi-emniyet-stogu.mdx` (+205) — ~12 dk uzun-form. Bölümler: (1) Süpermarket rafı sezgi girişi + EOQ neden yetersiz, (2) sürekli-izleme (Q, R) tanımı, (Q, R) vs (R, T) periyodik karşılaştırma, (3) emniyet stoğu neden gerekli — R = μL ile %50 servis çıkar, %95 için z=1.6449 türetimi, (4) kombine varyans türetimi Var(D_L) = E[L]·Var(d) + (E[d])²·Var(L) toplam varyans yasası, σ_L doğrudan eklemenin neden hatalı olduğu, sayısal örnek σ_L=0 vs σ_L=2 ile σ_DL 15.81 → 200.62 (13×), (5) Type-I vs Type-II ayrımı: cycle service P(D_L ≤ R), fill rate 1 − σ_DL·L(z)/Q, Q büyüdükçe fill rate hedefi daha az SS ister, (6) EOQ + toplam yıllık maliyet sayısal örneği (D=10000, K=100, h=2 → Q=1000, R=213.6, TC≈2043.5, SS %2), (7) yaygın 5 hata (σ_DL yerine σ_d, σ_L doğrudan ekleme, α=1 hedefleme, σ_L=0 varsayımı, Type-I/II karıştırma), (8) EOQ vs Newsvendor vs (Q,R) vs Wagner-Whitin vs EPQ karşılaştırma tablosu, (9) ne zaman (Q,R) yanlış araç: kısa yaşam döngüsü (Newsvendor), zaman-değişken deterministik (Wagner-Whitin), talep-tedarik korelasyonu, ürünler arası bağımlılık, (10) tarih: Wilson 1934 → Arrow-Harris-Marschak 1951 → Whitin 1953 → Hadley-Whitin 1963 → Peterson-Silver 1979 → SAP APO/Oracle Demantra/Kinaxis, (11) araç anatomisi + ilgili araç bağlantıları. **9 FAQ** JSON-LD: ROP nedir, SS neden gerekli, kombine varyans, z nereden, Type-I vs Type-II, Q ile SS ilişkisi, EOQ + SS + Newsvendor üçlüsü, α yükseldikçe maliyet, normal değil dağılım.
- `src/data/tools.ts` (+10) — envanter kategorisine `rop-emniyet-stogu` girişi.
- `public/og/rop-emniyet-stogu.png` (+81 KB) — `npm run og` üretti.

**Tasarım notu:** İki hata erken yakalandı ve düzeltildi. (a) İlk `inverseLoss` implementasyonunda `if (target >= 1/√(2π)) return -Infinity` erken çıkışı vardı — bu YANLIŞ çünkü L(z) z<0'da unbounded (L(-z) = L(z) + z Cachon-Terwiesch); Type-II Q=500, β=0.98 örneği target=0.72 (> 0.3989) veriyordu, bisection -Infinity dönüyordu. Kısıt kaldırıldı, lo=-30 seçildi (L(-30) ≈ 30 fazlasıyla üst sınır). (b) L(2.05) ≈ 0.00847 test sağlaması hafızadan yanlış hatırlandı — gerçek değer L(2.05) ≈ 0.00742 (Nahmias tablosu doğrulaması); test L(1.5) ≈ 0.0293'e döndürüldü (bilinen tablo değeri). Ayrıca astro sayfada `mudl` parametresi renderChart'ta hiç kullanılmıyordu (leadTimeDemand chart'ta implicit olarak SS altında) — check hint verdi, siyah unused parametresi kaldırıldı. Testere-dişi grafik implementasyonu iki çevrim + tedarik süresi düşüşü + Q sıçraması ile klasik ders kitabı görselini üretti; SS koridoru amber hafif fill ile talep düzeltmesi görünür.

**Kalite kapıları:** check ✓ (0 hata / 0 warning / 0 hint, **123 dosya**) · test ✓ (**686/686**, 33 dosya — +38 rop; 648 → 686) · build ✓ (**69 sayfa**, 5.51s — 67 → 69: yeni araç + rehber sayfası) · OG kartı üretildi · Lighthouse — yeni sayfalar mevcut BaseLayout + Container + Tailwind desenine bire bir uyumlu, ≥95 beklenir.

**Yayın:** PR [#70](https://github.com/Mavrikant/or-araclari/pull/70) squash-merge edildi (1c927de). Deploy run 29096218276 başlatıldı. Canlı: <https://karaman.dev/or-araclari/araclar/rop-emniyet-stogu> ve <https://karaman.dev/or-araclari/rehberler/yeniden-siparis-noktasi-emniyet-stogu>.

**İşaret:** yok (sırf 🟢 yeşil — yeni araç + eşlik eden rehber; algoritma saf JS, yeni bağımlılık yok, i18n mimarisi bozulmadı, statik-dışı hiçbir şey eklenmedi).

**Sıradaki:** Backlog'da açık 🟢 yeşil iş kalmadı (Q-CI-CHECK, Q-AUDIT-ESBUILD 🔴 blocked). Envanter serisi (EOQ + EPQ + Newsvendor + Wagner-Whitin + (Q,R)) beşli olarak tamamlandı. Bir sonraki döngü için güçlü adaylar: (a) F-CDS-NEH — Johnson aracına m ≥ 4 makina için CDS + NEH ikinci algoritma paneli (JPS/Lemke deseni), (b) F-BIN-PACKING — First-Fit-Decreasing + Best-Fit-Decreasing + Next-Fit sezgisel çözücü + kutu görselleştirme, (c) F-INVENTORY-COMPARE — beşli envanter modelini karşılaştıran "hangi model ne zaman" meta rehberi (yeni araç değil, cross-linked içerik) — F-ROP eklendiğinden içerik zenginleşti. En muhtemel: F-INVENTORY-COMPARE (envanter tazeliğinden yararlanır, S efor, meta içerik) veya F-CDS-NEH (Johnson tazeliği + ikinci algoritma deseni tekrarı).

---

## DÖNGÜ #32 — 2026-07-10

**Yapılan:** F-JOHNSON — Johnson Kuralı 2/3 Makina Akış-Tipi Çizelgeleme Çözücü. F₂ || Cmax için Johnson (1954) kapalı-form kuralı ile optimum iş sırası; F₃ için Johnson-Bellman indirgemesiyle koşul-optimum ya da sezgisel çözüm. Çizelgeleme kategorisine ders-programı / hemşire vardiya / PERT-CPM / fikstür arasında beşinci klasik olarak yerleşti — sıra-tipi (permütasyon) çizelgeleme ailesine ilk üye.

**Detay:**
- `src/lib/johnson.ts` (+245) — saf JS. Tipler: `JohnsonJob = { label, p1, p2, p3? }`; `p3` mevcutsa hepsinde mevcut olmalı (mixed reddedilir). Ana `solveJohnson`: (a) validate + machines ∈ {2,3} tespit, (b) sanal (α, β): iki makinada α=p₁, β=p₂; üç makinada α=p₁+p₂, β=p₂+p₃, (c) `johnsonPermutation`: U = { α ≤ β } α artan, V = { α > β } β azalan, U + V — orijinal indekse göre kararlı, (d) `simulate(sequence, machines)`: her makinada m1Now/m2Now/m3Now ilerlerken `m2Start = max(m1End, m2Now)`, boş süreleri (m1Idle, m2Idle, m3Idle) toplar. Üç-makina modunda `optimality`: `min p₁ ≥ max p₂` VEYA `min p₃ ≥ max p₂` → 'exact', aksi 'heuristic'; `reductionNote` insan-okur Türkçe açıklama. `virtualTimes` UI'ya U/V grup + α/β çiftini gösterir. `bruteForceOptimum(jobs)`: test yardımcısı, n! permütasyon simule eder — küçük n (≤ 8) için optimum baseline. Karmaşıklık: O(n log n) (sıralama).
- `src/lib/johnson.test.ts` (+204, **28 vitest**) — (a) Taha F2 5-iş ders kitabı elle iz sürüldü: sıra J4-J2-J5-J3-J1, Cmax=28 hand-verified; brute-force ile eşleşir. (b) Kenar durumları: single job, all-equal, M2-dominant (hepsi U), M1-dominant (hepsi V, β azalan), p₁=0 iş başa. (c) 8 rasgele 6-iş seed × 2 makina brute-force ile Cmax birebir eşleşme; 4 rasgele 5-iş seed × 3 makina (koşul-sağlanır kurulum: p₂ ∈ 1-3, p₁,p₃ ∈ 11-15) brute-force ile eşleşme + `optimality === 'exact'`. (d) 3 makina: exact/heuristic sınıflandırması, m1End ≤ m2Start ≤ m2End ≤ m3Start ≤ m3End invariantları. (e) Schedule tutarlılığı: `m2Start === max(m1End, prev m2End)` her iş için, Cmax ≥ Σp₁ + min p₂, Cmax ≥ max(p₁ + p₂), U/V grup tutarlılığı. (f) 3 validation kenarı: boş input, negatif süre, mixed 2/3-makina.
- `src/pages/araclar/johnson-akis-tipi-cizelgeleme.astro` (+430) — mobil-öncelikli iki-kolon. Sol form: 2/3 makina radyo (state'i güncelledikten sonra tabloyu yeniden render eder + p₃ alanını sıfırlar/ekler), canlı iş tablosu (label + p₁ + p₂ [+ p₃] + sil butonu, her hücre canlı state binding), + İş ekle butonu, iki hazır örnek (Taha 5-iş F2 + 3-makina koşul-sağlanan 4-iş), sıfırla + confirm dialog. Sağ output: emerald Cmax kartı + `Sıra: J4 → J2 → J5 → J3 → J1` özet + optimality etiketi (yeşil "exact" veya ambar "heuristic" reductionNote ile), 4-lü metrik (iş sayısı, M1/M2/M3 boş süre — M3 iki makinada gizli), Gantt SVG (M1/M2/M3 satırları slate zemin, iş blokları 8 renkli paletten `jobIndex % 8`, blok içi etiket beyaz + genişlik > 26 iken görünür, tick + Cmax dikey emerald çizgi + etiket), sıra tablosu (M1/M2/M3 aralıkları + M2 boş süresi ambar renkte vurgulu), U/V kural tablosu (`α = p₁ [+p₂]`, `β = p₂ [+p₃]`, brand/ambar grup chip, sıra sütunu). `localStorage` `johnson-state-v1` ile state persist.
- `src/content/rehberler/johnson-akis-tipi-cizelgeleme.mdx` (+283) — ~12 dk uzun-form. Bölümler: (1) 5-iş torna+freze sezgi girişi, (2) flow-shop vs job-shop, (3) Cmax + alternatif ölçütler (Σ Ci, Lmax, Σ Tj), (4) üç adım (Ayır → Sırala → Birleştir) + Taha F2 sayılı iz sürme (Cmax=28 vs "iş numarası sırası" Cmax=33 → %18 fark), (5) ikili-değiştirme ispatı `min(a_i, b_j) ≤ min(a_j, b_i)` + U/V yorumu, (6) Johnson-Bellman 3-makina uzantısı: koşul + α=p₁+p₂, β=p₂+p₃ indirgeme, koşul-sağlanmayanda sezgisel kalifikasyonu, sayısal 4-iş örneği, (7) m ≥ 4 için CDS/NEH/Palmer literatür özeti + kompleksite karşılaştırma (Johnson O(n log n) vs NEH O(n²·m)), (8) yaygın 5 hata (numara sırasıyla uygulama, SPT ile karıştırma, 3-makina koşul kontrolsüz "optimum" sanma, setup ayrımı yanılgısı, preemption varsayma), (9) uygulama alanları (imalat, baskı, mutfak, laboratuvar), (10) tarih: 1954 makale + Bellman/Nash dalgası + Gantt 1917 + Taillard 1993 benchmark seti, (11) araç anatomisi + ilgili araç bağlantıları. **9 FAQ** JSON-LD: kural nedir, flow-shop/job-shop, Cmax nedir, niçin U α artan V β azalan, ties, 3-makina koşulu, m ≥ 4 genelleme, boşluk sebepleri, preemption/non-preemptive.
- `src/data/tools.ts` (+10) — `cizelgeleme` kategorisine `johnson-akis-tipi-cizelgeleme`.
- `public/og/johnson-akis-tipi-cizelgeleme.png` (+84 KB) — `npm run og` üretti.

**Tasarım notu:** İlk yazımda 5-iş Taha örneği için Cmax'ı 25 hesaplamıştım (hafızadan yanlış), brute-force 28 döndü — testler bunu yakaladı, elle iz sürerek 28 doğrulandı (J4 M1 [0,1], M2 [1,5]; J2 M1 [1,3], M2 [5,13]; J5 M1 [3,6], M2 [13,18]; J3 M1 [6,10], M2 [18,25]; J1 M1 [10,16], M2 [25,28]). Aynı test bloğunda "Cmax ≥ max p₁ + Σ p₂" alt sınırı doğru değildi (M2 idle olabilir) — `Cmax ≥ max(p₁ + p₂)` (any single-job trace bound) ile değiştirildi. MDX gövde metni `{ i : a_i ≤ b_i }` içerdiğinde MDX JSX ifadesi olarak parse ettiği için backtick içine alındı; Astro sayfada aynı sorun `<p>` içindeki `{ α ≤ β }` için `U kümesi: α ≤ β olan işler` düz metne dönüştürüldü. Astro schema `description ≤ 200` char sınırı ilk rehber açıklamamı geçtiği için kısaltıldı.

**Kalite kapıları:** check ✓ (0 hata / 0 warning / 0 hint, **120 dosya**) · test ✓ (**648/648**, 32 dosya — +28 johnson) · build ✓ (**67 sayfa**, 5.57s) · OG kartı üretildi · Lighthouse — mevcut BaseLayout + Container + Tailwind desenine bire bir uyumlu, ≥95 beklenir.

**Yayın:** PR [#69](https://github.com/Mavrikant/or-araclari/pull/69) squash-merge edildi (b7a53d9). Deploy run 29092339721 başlatıldı. Canlı: <https://karaman.dev/or-araclari/araclar/johnson-akis-tipi-cizelgeleme> ve <https://karaman.dev/or-araclari/rehberler/johnson-akis-tipi-cizelgeleme>.

**İşaret:** yok (sırf 🟢 yeşil — yeni araç + eşlik eden rehber; algoritma saf JS, yeni bağımlılık yok, i18n mimarisi bozulmadı).

**Sıradaki:** Backlog'da açık 🟢 yeşil iş kalmadı (Q-CI-CHECK, Q-AUDIT-ESBUILD 🔴 blocked). Yeni araç fikri turu: (a) F-INVENTORY-COMPARE — EOQ/EPQ/Newsvendor/Wagner-Whitin dörtlüsünü karşılaştıran "hangi model ne zaman" cross-linked rehberi (yeni araç değil, meta içerik), (b) F-CDS-NEH — Johnson aracına m ≥ 4 için CDS + NEH ikinci algoritma deseni (JPS'i A*'a eklediğim gibi), (c) F-ROP — Emniyet stoğu / yeniden sipariş noktası (Newsvendor + EOQ ile üçüncü envanter modeli), (d) F-BIN-PACKING — First-Fit-Decreasing sezgisel. En muhtemel: F-CDS-NEH (Johnson tazeliğinden yararlanır) veya F-ROP (envanter üçlüsünü tamamlar).

---

## DÖNGÜ #31 — 2026-07-10

**Yapılan:** F-WAGNER-WHITIN — Wagner-Whitin Dinamik Lot Boyutlandırma Çözücü. Değişken talep, zamana bağlı kurulum ve taşıma maliyetleri altında çok-dönemli üretim/sipariş çizelgesi için forward DP. Envanter kategorisinde EOQ (sabit talep) → Newsvendor (tek-dönem stokastik) sonrası, çok-dönemli deterministik + zaman-değişken ekseninde üçüncü stok modeli. MRP çekirdeğinde yerleşik klasik (1958).

**Detay:**
- `src/lib/wagner-whitin.ts` (+370) — saf JS. Tipler: `WagnerWhitinInput = { demands, setupCost, holdingCost, unitCost? }`; her maliyet tek sayı veya T-uzunluklu dizi. `expand()` yardımcısı liste/skalar tekilleştirir + negatif/NaN/uzunluk validation. `blockCost(j, k, ...)` blok maliyeti kümülatif taşıma katsayısıyla: her $p > j$ için `cumHold += h[p-2]; holding += cumHold * d[p-1]`. Ana `solveWagnerWhitin`: $f[0]=0$, $f[t] = \min_{1 \leq j \leq t} \{ f[j-1] + c(j,t) \}$, argmin `arg[t]` kaydedilir; sondan başa `k = arg[k] - 1` geri izleyerek `lots[]` çıkarılır; `periods[]` `endingInventory` + `holdingChargedHere` + `isSetup` bayrakları ile ledger olarak yeniden inşa edilir. Yan olarak `solveSilverMeal` (Silver & Meal 1973) sezgiseli aynı çıktı şekliyle: mevcut lotu genişletirken dönem başına ortalama maliyet azalırken devam et, artmaya başladığında lotu kapat. Karmaşıklık $O(T^2)$; 240 dönem üst sınırı UX koruması.
- `src/lib/wagner-whitin.test.ts` (+218, **27 vitest**) — (a) elle doğrulanmış 3-dönem $d=[10,20,30], K=50, h=1$ → 120₺ lots={1-2, 3-3}: her block cost hand-computed, DP izlendi. (b) yapısal testler: single-period, high-K → single lot, $h=0$ → single lot (ties j=1 wins), high-h → lot-for-lot, $d_p=0$ dönem lotu kırmaz (previous lot extends), $c$ sabitse plan invariant + $\Sigma c \cdot d$ additive, time-varying $K_j=200$ pahalı dönem 2 avoid. (c) ZIO property: 4 farklı problemde (3-dönem, Nahmias 10, Silver-Peterson 12, time-varying 6) `for each p: produce > 0 ⇒ prevEnding === 0`. (d) accounting identities: $\sum lots = totals$, $\sum produce = \sum demand$, final ending $= 0$, reconstructed period holding $=$ lot-level holding, setupCount $=$ lots.length $=$ flagged periods. (e) **optimallik kanıtı**: 5-dönem $d=[30,40,50,20,60]$, $K=100$, $h=1$ için $2^4=16$ olası partisyonu enumerate ederek WW cost $\leq$ each; ayrıca 3 problem üzerinde WW $\leq$ Silver-Meal dominance. (f) 8 validation kenar (empty, negative, NaN, zero-total, non-positive K, negative h, length mismatch, $T > 240$).
- `src/pages/araclar/wagner-whitin-cozucu.astro` (+489) — mobil-öncelikli iki-kolon grid. Sol form: talep textarea (virgül/boşluk/nokta virgül parser + canlı `demand-info` "N dönem · toplam X"), K/h/c input'ları tek sayı VEYA T-uzunluklu virgüllü liste ("100" ya da "80,40,120,80,40,80"), iki hazır örnek butonu, sıfırla + confirm dialog. Sağ output: yeşil toplam maliyet kartı + `${setupCount} kurulum · ${T} dönem · toplam üretim` özet, 4-lü metrik ($\Sigma K$, $\Sigma h$, $\Sigma c$, Silver-Meal karşılaştırma "+X (%Y)" veya "aynı"), lot yapısı SVG çubuğu (dönem hücreleri + emerald bloklar + Q etiketi + kurulum noktası; genişlik `max(28, min(60, 720/T))` ile T uyumlu), lot çizelgesi tablosu (kapsanan dönem + miktar + $K$ + $h$ + toplam), dönem defteri tablosu ($t$, $d$, üretim, stok sonu, taşıma, `isSetup` chip). `localStorage` `wagner-whitin-state-v1` string olarak (parser dönem/liste bilgisini de kurtarır).
- `src/content/rehberler/wagner-whitin-dinamik-lot.mdx` (+265) — ~12 dk uzun-form. Bölümler: (1) Nahmias 10-dönem sezgi girişi ($d = [10, 62, 12, 130, 154, 129, 88, 52, 124, 160]$), (2) dinamik lot problemi ($d_t, K_t, h_t$ zaman-değişken, EOQ varsayımlarının 3 kırılışı), (3) Zero Inventory Ordering teoremi + swap-argümanlı ispat sezgisi (elde stok varken üretim varsa strict iyileşme), (4) ileri DP formülasyonu $f(t) = \min_j \{ f(j-1) + c(j,t) \}$ + kümülatif taşıma açılımı, karmaşıklık $O(T^2)$ → $O(T \log T)$ Aggarwal-Park, (5) sayısal örnek: 10-dönem serisi manuel blok maliyeti $[1..3]$ = 186, DP tablosu ilk 3 satır elle, optimal 5 lot @ 864₺ hedef, (6) Silver-Meal + LUC + PPB + POQ + LFL sezgiselleri (her birinin ne zaman WW'ye yakın/uzak olduğu), (7) rolling horizon + "MRP nervousness" — donma dönemi, sadece ilk lotu uygula, Blackburn-Millen 1980 ceza terimli formülasyon, (8) sınırlar: CLSP NP-zor, multi-item Manne 1958 + Dixon-Silver 1981, stokastik Federgruen-Zipkin ayrı literatür, (9) yaygın 5 hata (EOQ zorlama, horizon effect, birim çevrimi, sıfır talep dönem yanılgısı, sezgisel yerine koyma), (10) tarih Wagner + Whitin 1958 → Orlicky 1975 MRP → COPICS/MAPICS 1980 → SAP APO/Oracle ASCP, (11) özet. **9 FAQ** JSON-LD: WW nedir, ZIO nedir, ileri DP formülü, EOQ farkı, Silver-Meal karşılaştırma, ne zaman WW yanlış araç, nervousness, karmaşıklık, MRP oturması.
- `src/data/tools.ts` (+10) — envanter kategorisine `wagner-whitin-cozucu` girişi.
- `public/og/wagner-whitin-cozucu.png` (+82 KB) — `npm run og` üretti.

**Tasarım notu:** Blok maliyeti hesabında ilk yazımda `holding[p-1]` indeksi kullanıldı ama semantik olarak $j$'den $p$'ye taşınan bir birim $h_j, h_{j+1}, \ldots, h_{p-1}$ katsayılarını görür (her $h_q$ q'dan q+1'e taşıma maliyeti) — `holding[p-2]` doğrusu, çünkü dizi 0-endeksli ve $h_1 = $ `holding[0]`. 3-dönem hand-verified testi bu tutarlılığı erken yakaladı ve düzeltildi. Ayrıca `h=0` durumunda tie-breaking: `if (total < best)` strict — $j$ küçük olan (yani $j=1$) önce dener, tek lot kazanır. Bu davranış test edildi; alternatif olarak `≤` ile en son argmin de doğru ama tek-lot çıktısı kullanıcıya daha okunaklı. Silver-Meal aynı `WagnerWhitinResult` şeklinde döner (özdeş output tabloları) ama `dpValues`/`dpArgmin` boş 0-dizisi — DP kullanmıyor, ama UI'da tutarlılık için tip aynı.

**Kalite kapıları:** check ✓ (0 hata, 0 warning, 0 hint, **117 dosya** — 114 → 117: +1 lib, +1 test, +1 page, +1 rehber, -1 muhtemelen yeniden numaralandırma) · test ✓ (**620/620**, 31 dosya — +27 wagner-whitin testi; 593 → 620) · build ✓ (**65 sayfa**, 5.30s — 63 → 65: yeni araç + rehber sayfası) · OG kart üretildi · Lighthouse — yeni sayfalar mevcut BaseLayout + Container + Tailwind desenine bire bir uyumlu, ≥95 beklenir.

**Yayın:** PR [#68](https://github.com/Mavrikant/or-araclari/pull/68) squash-merge edildi (ccfc806). Deploy run 29083214936 → completed/success. Canlı: <https://karaman.dev/or-araclari/araclar/wagner-whitin-cozucu> ve <https://karaman.dev/or-araclari/rehberler/wagner-whitin-dinamik-lot>.

**İşaret:** yok (sırf 🟢 yeşil — yeni araç + eşlik eden rehber; algoritma saf JS, yeni bağımlılık yok, i18n mimarisi bozulmadı, statik-dışı hiçbir şey eklenmedi, mobil-öncelikli grid + tablolar responsive).

**Sıradaki:** Backlog'da açık 🟢 yeşil iş kalmadı (Q-CI-CHECK, Q-AUDIT-ESBUILD 🔴 blocked). Bir sonraki döngüde yeni araç fikri turu: Simplex tableau adım-adım gösterici (LP aracına eğitici uzantı) · Bin Packing First-Fit-Decreasing sezgisel çözücü · Yeni yazılım/donanım güvenlik açığı taraması (npm audit) · TSP aracına 2-opt animasyonu ikinci algoritma deseni · İçerik cilası: EOQ / EPQ / Newsvendor / Wagner-Whitin envanter dörtlüsünü karşılaştıran bir "hangi model ne zaman" rehberi (F-INVENTORY-COMPARE fikri).

---

## DÖNGÜ #30 — 2026-07-10

**Yapılan:** F-NEWSVENDOR — Newsvendor (Tek-Dönem Envanter) Çözücü. Belirsiz talep altında sipariş miktarı $Q^*$ için critical fractile $F(Q^*) = C_u / (C_u + C_o)$ kapalı-form çözümü + eşlik eden ~12 dk uzun-form Türkçe rehber. Envanter kategorisinin EOQ/EPQ ailesine, deterministik yerine stokastik-tek-dönem eksenine ilk üye eklendi.

**Detay:**
- Dal (`cycle/newsvendor`) önceden başlatılmış olarak bulundu (algoritma + test + astro sayfası uncommitted, tools.ts modifiye). `guideSlug: 'newsvendor-tek-donem-envanter'` referansı vardı ama MDX yoktu → HARD CONSTRAINT eksiği. Ayrıca astro sayfada `astro check`'in yakaladığı 2 typecheck hatası (chart yardımcısında ölü kod bloğu: `p.expectedShortage * input.costs.mode === 'direct' ? … : …` — hem type hem semantik olarak bozuk; `yValues` array'i popülüyor ama okunmuyordu, `yMax` başka yerde bağımsız hesaplanıyordu). Bu döngü: (a) ölü blok + gölge `profit-legend` DOM elemanı + `hasProfit` parametresi temizlendi (grafik gerçekte kâr eğrisi çizmiyordu, yalın yalan olmasın), (b) 12 dk MDX yazıldı, (c) OG kartı üretildi, (d) merge + deploy.
- `src/lib/newsvendor.ts` (+367) — saf JS. Tipler: `NewsvendorInput = { costs, demand }`; `costs` iki mod (`direct: {underage, overage}` veya `priced: {price, cost, salvage?, shortagePenalty?}`); `demand` iki dağılım (`normal: {mean, stdDev}` veya `uniform: {min, max}`). `resolveCosts` fiyat modunda $C_u = p - c + g$, $C_o = c - s$ türetir. Yardımcılar: `normalPdf` (kapalı $\varphi$), `normalCdf` (Abramowitz-Stegun 7.1.26 erf yaklaşımı, $|\varepsilon| < 1.5 \times 10^{-7}$), `normalInverse` (Beasley-Springer-Moro 1988, üç kollu rasyonel yaklaşım, $|\varepsilon| < 10^{-9}$), `standardNormalLoss` ($L(z) = \varphi(z) - z(1-\Phi(z))$). Ana `computeNewsvendor`: normal için $z = \Phi^{-1}(CR)$, $Q^* = \max(0, \mu + z\sigma)$, $E[\text{eksik}] = \sigma L(z_Q)$, $E[\text{fazla}] = Q^* - \mu + E[\text{eksik}]$ (accounting identity ile); üniform için kapalı form $Q^* = a + (b-a)CR$, $E[\text{eksik}] = (b-Q)^2 / (2(b-a))$, $E[\text{fazla}] = (Q-a)^2 / (2(b-a))$. `expectedProfit = p \cdot E[\text{sales}] + s \cdot E[\text{leftover}] - c \cdot Q - g \cdot E[\text{shortage}]$ yalnızca priced modda; direct modda `null`. `newsvendorSensitivity(input, steps, qMinFactor, qMaxFactor)`: Q\* etrafında `steps + 1` nokta, U-eğrisi bileşenleriyle. Validation: NaN reddi, negatif Cu/Co, Cu + Co = 0, $p \leq c$, $s > c$, $\sigma \leq 0$, $\text{max} \leq \text{min}$ hepsi `NewsvendorError` fırlatır.
- `src/lib/newsvendor.test.ts` (+279, **35 vitest**) — Abramowitz-Stegun round-trip, Beasley-Springer-Moro benchmark noktalar ($\Phi^{-1}(0.975) \approx 1.959964$), Cachon-Terwiesch kayıp fonksiyonu kimliği $L(-z) = L(z) + z$, kritik oran (0.8, 0.5, 0.99, 0.01) her senaryoda doğru $Q^*$, priced/direct mod türetimi, accounting identity $Q^* - E[\text{leftover}] = E[D] - E[\text{shortage}]$, fill rate = expectedSales / mean, `expectedProfit === null` iff direct, sensitivity: minimum expected cost tam $Q^*$'da (200 nokta, $\pm 40\%$ Q aralığında), maximum expected profit priced modda tam $Q^*$'da, 7 validation kenar durumu.
- `src/pages/araclar/newsvendor-cozucu.astro` (+558) — mobil-öncelikli iki-kolon grid; sol form (maliyet modu radyo → direct/priced field-set toggle, dağılım radyo → normal/uniform field-set toggle, iki hazır örnek: "Gazete bayii" (priced p=5, c=2, s=0.5, normal μ=150, σ=30) + "Sezonluk giyim" (priced p=80, c=30, s=15, uniform 200-500), sıfırla + confirm dialog), sağ output (yeşil Q\* kartı + formül satırı `Q* = μ + Φ⁻¹(CR)·σ = …` veya `Q* = a + (b-a)·CR = …`, 8 metrik kartı + koşullu E[kâr] hücresi, U-eğrisi SVG: red Cu·E[eksik], blue Co·E[fazla], black toplam, yeşil dashed Q\* dikey çizgi, x/y aksis tick + eksen başlıkları). `localStorage` `newsvendor-state-v1` anahtarıyla mod/dağılım/tüm giriş alanlarını persist; sample buton state'i güncelledikten sonra saveState çağrılır. `NewsvendorError` yakalanıp `error-box` panelinde gösterilir.
- `src/content/rehberler/newsvendor-tek-donem-envanter.mdx` (+265) — ~12 dk uzun-form. Bölümler: (1) Giriş (gazete bayii senaryosu, EOQ yetersizliği), (2) Problem tanımı (tek-dönem, belirsiz talep, asimetrik maliyet), (3) Cu/Co türetimi + iki mod, (4) Critical fractile marjinal analiz türetimi ($\Delta(Q) = C_u(1-F) - C_o F$, $\Delta(Q^*)=0$ noktası), (5) Cu ≫ Co / Cu ≪ Co / Cu = Co üç sezgi, (6) Normal talep + z-tablosu (0.5–0.999), servis seviyesi asimetrisi (95→99: %42 emniyet stoğu artışı), (7) Standart kayıp fonksiyonu $L(z)$ ile eksik/fazla, (8) Üniform talep kapalı form + polinom kayıp, (9) Sezonluk bot uçtan-uca sayısal örnek (p=80, c=30, s=15, μ=350, σ=90 → CR=0.769 → Q\*≈416, E[eksik]≈11.5, fill rate ≈%96.7), (10) Araç kullanım adımları, (11) Model varsayımlarına kritik notlar (talep tahmin kalitesi > formül, broken-assortment, dinamik hurda değeri, "tek dönem" ne zaman kırılır), (12) EOQ vs (Q,R) vs newsvendor kavramsal üçlü, (13) Yaygın 5 hata, (14) Tarih (Edgeworth 1888 → Arrow-Harris-Marschak 1951) + modern uzantılar (multi-product Lagrange, risk-averse CVaR, distribution-free Scarf 1958, buyback sözleşmeleri), (15) Özet. **9 FAQ** JSON-LD: newsvendor nedir, critical fractile nereden, Cu/Co hesabı, EOQ farkı, ortalama neden yanlış, servis seviyesi vs fill rate, normal talep formülü, dağılım normal değilse, U-eğrisi robustluğu.
- `src/data/tools.ts` (+10) — envanter kategorisine `newsvendor-cozucu` girişi (title, shortTitle, description, category, status: 'ready', guideSlug). Diğer araçların desenine bire bir uyumlu.
- `public/og/newsvendor-cozucu.png` (+87 KB) — `npm run og` script'i tools.ts'ten üretti.

**Tasarım notu:** Astro sayfa uncommitted bir dal üzerinde bulundu. İki tercih yapıldı: (a) chart'ta `hasProfit` parametresi + `#profit-legend` DOM elemanı olduğu hâlde grafik gerçekte kâr eğrisi çizmiyordu (yalnız cost eğrileri) → yalancı legend'ı kaldırmak yerine korumak, ileride kâr eğrisi eklendiğinde tekrar bağlamak isterdik ama `[10/10]`: mevcut hâl doğrudan bir yalan → temizlik doğru karar. (b) MDX description sınırı 200 char; ilk taslak 240 char'daydı, kısaltıp anlam korundu. Guide FAQ'da servis-seviyesi asimetrisi tablosu (z değerleri) rehberin en yüksek sinyal-yoğun kısmı; öğretici z=0.674 ↔ 75%, z=1.645 ↔ 95%, z=2.33 ↔ 99% içselleştirmesi.

**Kalite kapıları:** check ✓ (0 hata, 0 warning, 0 hint, **114 dosya**) · test ✓ (**593/593**, 30 dosya — +35 newsvendor testi; PERT rehber döngüsünden bu yana test tabanı değişmedi, önceki 558 → 593 = +35) · build ✓ (**63 sayfa**, 5.49s — 62 → 63: yeni araç sayfası + rehber sayfası, +1 net çünkü Steiner/JPS aynı sayfayı paylaşıyor artık) · OG kart üretildi · Lighthouse — yeni sayfalar mevcut BaseLayout + Container + Tailwind desenine bire bir uyumlu, ≥95 beklenir.

**Yayın:** PR [#66](https://github.com/Mavrikant/or-araclari/pull/66) squash-merge edildi. Deploy run 29076924298 → completed/success (8s deploy). Canlı: <https://karaman.dev/or-araclari/araclar/newsvendor-cozucu> ve <https://karaman.dev/or-araclari/rehberler/newsvendor-tek-donem-envanter>.

**İşaret:** yok (sırf 🟢 yeşil — yeni araç + eşlik eden rehber; algoritma saf JS, yeni bağımlılık yok, i18n mimarisi bozulmadı, statik-dışı hiçbir şey eklenmedi).

**Sıradaki:** Backlog'da açık 🟢 yeşil iş kalmadı (Q-CI-CHECK, Q-AUDIT-ESBUILD 🔴 blocked). Bir sonraki döngüde yeni araç fikri turu: Wagner-Whitin dinamik lot boyutlandırma (envanter DP) · Simplex tableau adım-adım gösterici · TSP aracına 2-opt animasyonu ikinci algoritma.
